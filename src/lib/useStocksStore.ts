...
interface StocksStore {
  ...
  // Expense Actions
  addExpense: (label: string, amount: number, userName: string, category: string) => void;
...
const DEMO_EXPENSES: Expense[] = [
  ...
  {
    id: "exp-2",
    label: "Réparation machine expresso",
    amount: 150,
...
export const useStocksStore = create<StocksStore>()(
  persist(
    (set) => ({
      ...
      addStock: (itemId, quantity, userName, details) =>
        set((state) => {
      ...
      deductStock: (itemId, quantity, userName, details) =>
        set((state) => {
          const transaction: StockTransaction = {
            id: `tx-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
            itemId,
      ...
      addStockItem: (name, unit, userName, initialQty = 0, categoryId) => {
        ...
        set((state) => {
          ...
          return {
            items: [
              ...state.items,
              { id, name, unit, initialStock: initialQty, usedToday: 0, categoryId },
            ],
            transactions: [transaction, ...(state.transactions || [])],
          };
        });
      ...
      addExpense: (label, amount, userName, category) =>
        set((state) => ({
          expenses: [
            {
              id: `exp-${Date.now()}`,
