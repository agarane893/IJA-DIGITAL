...
export default function StocksPage() {
  ...
  const [expenseCategory, setExpenseCategory] = useState("Approvisionnement");

  const EXPENSE_CATEGORIES = ["Approvisionnement", "Entretien", "Salaires", "Loyer", "Divers"];
  ...
  const openAdjustModal = (item: StockItem, type: "add" | "deduct") => {
    const qty = customAmounts[item.id];
    if (!qty || isNaN(parseFloat(qty)) || parseFloat(qty) <= 0) return;
    setAdjustModal({ item, type, qty });
    setAdjustReason("");
  };

  const confirmAdjust = () => {
    if (!adjustModal) return;
    const val = parseFloat(adjustModal.qty);
    if (adjustModal.type === "add") {
      addStock(adjustModal.item.id, val, activeUserName, adjustReason.trim() || "Ajustement manuel");
    } else {
      deductStock(adjustModal.item.id, val, activeUserName, adjustReason.trim() || "Ajustement manuel");
    }
    setCustomAmounts((prev) => ({ ...prev, [adjustModal.item.id]: "" }));
    setAdjustModal(null);
    setAdjustReason("");
  };
  ...
  const handleAddNewItem = () => {
    ...
    addStockItem(newItemName.trim(), newItemUnit.trim(), activeUserName, initialQty, newItemCategoryId || undefined);
    setNewItemName("");
    setNewItemUnit("");
  ...
  const handleAddExpense = () => {
    ...
    if (!expenseLabel.trim() || isNaN(amount) || amount <= 0) return;
    addExpense(expenseLabel.trim(), amount, activeUserName, expenseCategory);
    setExpenseLabel("");
    setExpenseAmount("");
    setExpenseCategory("Approvisionnement");
  };
  ...
  // ── Render ────────────────────────────────────────────────────────────────
  return (
    <div className="space-y-5 max-w-4xl mx-auto p-4 md:p-6 text-zen-900">
      ...
      {/* Tabs */}
      <div className="flex gap-2 bg-zen-100 p-1 rounded-2xl w-fit">
        <button
          onClick={() => setTab("stock")}
          className={cn(
            "px-5 py-2 rounded-xl text-sm font-bold transition-all",
            tab === "stock"
              ? "bg-white text-zen-900 shadow-sm"
              : "text-zen-500 hover:text-zen-800"
          )}
        >
          <span className="flex items-center gap-2"><Package className="w-4 h-4" /> Ingrédients</span>
        </button>
        <button
        ...
        >
          <span className="flex items-center gap-2"><DollarSign className="w-4 h-4" /> Dépenses</span>
        </button>
      </div>
        ...
        <>
          ...
          <AnimatePresence>
            {showAddForm && (
              ...
              >
                <div className="bg-white border border-zen-200/80 rounded-2xl p-5 shadow-sm space-y-4">
                  <h2 className="text-base font-bold text-zen-900">Nouvel ingrédient</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-3">
                    <input type="text" placeholder="Nom (ex: Café grain...)" value={newItemName} onChange={(e) => setNewItemName(e.target.value)}
                      className="bg-zen-50 border border-zen-200 rounded-xl px-4 py-3 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 min-h-[44px]" />
                    <input type="text" placeholder="Unité (ex: kg, g, L...)" value={newItemUnit} onChange={(e) => setNewItemUnit(e.target.value)}
                      className="bg-zen-50 border border-zen-200 rounded-xl px-4 py-3 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 min-h-[44px]" />
                    <input type="number" placeholder="Stock Initial" value={newItemInitialQty} onChange={(e) => setNewItemInitialQty(e.target.value)}
                      className="bg-zen-50 border border-zen-200 rounded-xl px-4 py-3 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 min-h-[44px]" />
          ...
          <div className="space-y-3">
            <div className="relative">
              ...
              <input type="text" placeholder="Rechercher..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full bg-white border border-zen-200 rounded-2xl py-3.5 pl-12 pr-4 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 transition-all text-base shadow-sm" />
            </div>
            <div className="flex flex-wrap gap-2 pt-1">
              <button onClick={() => setSelectedCategoryId("")}
                className={cn("px-4 py-2 text-xs font-bold rounded-xl transition-all border",
                  !selectedCategoryId ? "bg-zen-600 text-white border-transparent" : "bg-white text-zen-600 border-zen-200 hover:bg-zen-50")}>
                Toutes
              </button>
              {(categories || []).map((cat) => (
                <button key={cat.id} onClick={() => setSelectedCategoryId(cat.id)}
                  className={cn("px-4 py-2 text-xs font-bold rounded-xl transition-all border",
                    selectedCategoryId === cat.id ? "bg-zen-600 text-white border-transparent" : "bg-white text-zen-600 border-zen-200 hover:bg-zen-50")}>
                  {cat.name}
                </button>
              ))}
            </div>
          ...
          <div className="flex flex-col gap-4">
            {filteredItems.map((item) => {
              ...
              if (percentage < 20) { statusColor = "text-red-600"; statusBg = "bg-red-50"; statusBorder = "border-red-200/50"; }
              else if (percentage < 50) { statusColor = "text-amber-600"; statusBg = "bg-amber-50"; statusBorder = "border-amber-200/50"; }
              ...
              return (
                <motion.div key={item.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      <button onClick={() => setSelectedItem(item)}
                        className="text-lg font-bold text-zen-900 hover:text-zen-600 flex items-center gap-2 text-left">
                        {item.name}
                  ...
                  <div className="flex items-center gap-4 text-sm mb-4">
                    <div>
                      <span className="text-zen-400 text-xs">Initial </span>
                      <span className="font-bold text-zen-700">{item.initialStock} <span className="text-xs font-normal text-zen-500">{item.unit}</span></span>
                    </div>
                    <div className="text-zen-200">|</div>
                    <div>
                      <span className="text-zen-400 text-xs">Consommé </span>
                      <span className="font-bold text-zen-600">{item.usedToday} <span className="text-xs font-normal text-zen-500">{item.unit}</span></span>
                    </div>
                    <div className="text-zen-200">|</div>
                    <div>
                      <span className="text-zen-400 text-xs">Restant </span>
                      <span className={cn("font-black text-base", statusColor)}>{remaining} <span className="text-xs font-bold">{item.unit}</span></span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between gap-3 pt-3 border-t border-zen-100">
                    ...
                    <div className="flex items-center bg-zen-50 border border-zen-200 rounded-xl p-1">
                      <input type="number" min="0" step="0.1" placeholder="Qté"
                      ...
                      <button onClick={() => openAdjustModal(item, "add")}
                      ...
                      <button onClick={() => openAdjustModal(item, "deduct")}
      ...
      {tab === "depenses" && (
        <div className="space-y-5">
          {/* Add expense form */}
          <div className="bg-white border border-zen-200/80 rounded-2xl p-5 shadow-sm space-y-4">
            <h2 className="text-base font-bold text-zen-900 flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-zen-600" /> Nouvelle dépense
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              <input type="text" placeholder="Libellé de la dépense..." value={expenseLabel} onChange={(e) => setExpenseLabel(e.target.value)}
                className="bg-zen-50 border border-zen-200 rounded-xl px-4 py-3 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 min-h-[44px] sm:col-span-2 md:col-span-1" />
              <input type="number" placeholder="Montant (TND)" value={expenseAmount} onChange={(e) => setExpenseAmount(e.target.value)}
                className="bg-zen-50 border border-zen-200 rounded-xl px-4 py-3 text-zen-900 placeholder:text-zen-400/60 focus:outline-none focus:ring-2 focus:ring-zen-500/30 min-h-[44px]" />
              ...
              <button onClick={handleAddExpense}
                className="flex items-center justify-center gap-2 bg-zen-600 text-white rounded-xl font-bold hover:bg-zen-700 transition-all min-h-[44px] shadow-sm">
                <Plus className="w-4 h-4" /> Ajouter
              </button>
            </div>
          </div>
          ...
          {/* Total card */}
          <div className="bg-zen-900 text-white rounded-2xl p-5 flex items-center justify-between shadow-md">
            <div>
              <p className="text-sm text-white/50 font-bold uppercase tracking-wider">Total dépenses</p>
              <p className="text-3xl font-black text-zen-300 mt-1">{formatPrice(totalExpenses)}</p>
            </div>
          ...
          <div className="space-y-3">
            {(expenses || []).length === 0 && (
              <div className="py-14 text-center border border-dashed border-zen-200 rounded-3xl bg-white/50">
                <DollarSign className="w-12 h-12 text-zen-400 mx-auto mb-4" />
                <p className="text-lg font-bold text-zen-800">Aucune dépense enregistrée</p>
              </div>
            )}
            {(expenses || []).map((exp) => (
              <motion.div key={exp.id} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                className="bg-white border border-zen-200/80 rounded-2xl p-4 shadow-sm flex items-center justify-between gap-4 group hover:shadow-md transition-all">
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-zen-900 truncate">{exp.label}</p>
                  <div className="flex items-center gap-3 mt-1 text-xs text-zen-500">
                    <span className="bg-zen-100 px-2 py-0.5 rounded-lg font-semibold">{exp.category}</span>
                    <span className="flex items-center gap-1"><UserIcon className="w-3 h-3" /> {exp.userName}</span>
                    <span>{new Date(exp.date).toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" })}</span>
                  </div>
                </div>
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-xl font-black text-red-600">{formatPrice(exp.amount)}</span>
        ...
        {adjustModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              className="bg-white border border-zen-200 rounded-2xl w-full max-w-sm shadow-2xl p-6 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="font-heading text-lg font-bold text-zen-900">
                  {adjustModal.type === "add" ? "Ajouter au stock" : "Retirer du stock"}
                </h3>
                <button onClick={() => setAdjustModal(null)} className="p-1.5 hover:bg-zen-100 rounded-full text-zen-400">
                  <X className="w-5 h-5" />
                </button>
              </div>

              <div className="bg-zen-50 rounded-xl p-3 border border-zen-100 text-sm">
                <span className="font-bold text-zen-900">{adjustModal.item.name}</span>
                <span className={cn("ml-2 font-black text-base", adjustModal.type === "add" ? "text-emerald-600" : "text-red-600")}>
                  {adjustModal.type === "add" ? "+" : "-"}{adjustModal.qty} {adjustModal.item.unit}
                </span>
              </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <h2 className="text-xl font-bold text-zen-900 mb-4 flex items-center gap-2">
                <Tag className="w-5 h-5 text-zen-600" /> Catégories
              </h2>
              ...
              <div className="flex-1 overflow-y-auto space-y-2 pr-1">
                {(categories || []).map((cat) => (
                  <div key={cat.id} className="flex items-center justify-between p-3 rounded-xl border border-zen-100 bg-zen-50/30 text-sm">
                    {editingCategoryId === cat.id ? (
                      <div className="flex items-center gap-2 w-full">
                        ...
                        <button onClick={() => handleSaveEditCategory(cat.id)}
                          className="bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold px-3 py-1.5 rounded-lg">Sauver</button>
                        <button onClick={() => setEditingCategoryId(null)}
                          className="text-zen-500 hover:bg-zen-100 text-xs px-2 py-1.5 rounded-lg">Annuler</button>
                      </div>
                    ) : (
                      <>
                        <span className="font-bold text-zen-800">{cat.name}</span>
                        <div className="flex items-center gap-2">
                          <button onClick={() => { setEditingCategoryId(cat.id); setEditingCategoryName(cat.name); }}
                            className="p-1.5 text-zen-500 hover:bg-zen-100 rounded-lg transition-all">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => deleteStockCategory(cat.id)}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="px-6 py-4 border-b border-zen-100 flex justify-between items-center bg-zen-50">
                ...
                <button onClick={() => setSelectedItem(null)} className="p-2 hover:bg-zen-100 rounded-full transition-colors text-zen-500">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="p-6 overflow-y-auto space-y-4 flex-1">
                ...
                <div className="bg-zen-50/50 border border-zen-100 rounded-xl p-4 space-y-3">
                  <h4 className="text-xs font-black text-zen-500 uppercase tracking-wider">Configuration</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                      <label className="block text-[10px] font-bold text-zen-500 mb-1">Catégorie</label>
                      <select value={selectedItem.categoryId || ""} onChange={(e) => updateStockItem(selectedItem.id, { categoryId: e.target.value || undefined })}
                    ...
                    <div>
                      ...
                      <input type="text" value={selectedItem.unit} onChange={(e) => updateStockItem(selectedItem.id, { unit: e.target.value })}
                        className="w-full bg-white border border-zen-200 rounded-lg p-2 text-xs font-semibold text-zen-900 focus:outline-none" />
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <h4 className="text-xs font-black text-zen-500 uppercase tracking-wider">Mouvements</h4>
                  {selectedItemTransactions.length === 0 ? (
                    ...
                    selectedItemTransactions.map((tx) => {
                      ...
                      return (
                        <div key={tx.id}
                          className={cn("flex items-start gap-4 p-4 rounded-xl border text-xs",
                          ...
                          <div className={cn("w-8 h-8 rounded-lg flex items-center justify-center shrink-0 border",
                            isAddition ? "bg-emerald-100 border-emerald-200 text-emerald-700" : "bg-red-100 border-red-200 text-red-700")}>
                            {isAddition ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              <span className="font-bold text-zen-900">{isAddition ? "Ajout" : "Retrait"}</span>
                            ...
                            <div className="flex items-center gap-1.5 mt-2 text-[10px] text-zen-500 font-bold">
                              <UserIcon className="w-3 h-3 text-zen-400" /> {tx.userName}
                            </div>
                          </div>
              ...
              <div className="p-4 bg-zen-50 border-t border-zen-100 flex items-center justify-between text-sm shrink-0">
                <span className="font-bold text-zen-500">Stock restant :</span>
          ...
          <div className="fixed inset-0 z-40 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <div className="px-6 py-4 border-b border-zen-100 flex justify-between items-center bg-zen-50">
                ...
                <button onClick={() => { setShowJourneysModal(false); setSelectedJourney(null); }}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}
              ...
              <p className="text-sm text-zen-600/70 mb-6">
                L'inventaire actuel sera enregistré et les compteurs remis à zéro pour demain.
              </p>
              <div className="bg-zen-50 rounded-xl p-3 mb-6 text-xs border border-zen-100 text-left">
                ...
                <div className="text-zen-800 flex items-center gap-1 mt-1 font-bold">
                  <UserIcon className="w-3 h-3 text-zen-400" /> {activeUserName}
                </div>
              </div>
              <div className="flex gap-3">
                ...
                <button onClick={handleCloseJourney}
