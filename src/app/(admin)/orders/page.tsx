...
export default function OrdersPage() {
  ...
  return (
    <div className="space-y-6 h-full flex flex-col">
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 shrink-0">
        <div>
          <h1 className="font-heading text-2xl font-black text-white flex items-center gap-2">
            <UtensilsCrossed className="w-7 h-7 text-[#D95D39]" />
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="bg-[#161B22] border border-white/10 rounded-xl pl-9 pr-4 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-[#D95D39] transition-all w-48 lg:w-64"
            />
          </div>
          ...
          <select 
            ...
            className="bg-[#161B22] border border-white/10 rounded-xl px-4 py-2 text-sm text-white/70 focus:outline-none focus:border-[#D95D39] transition-all appearance-none cursor-pointer"
          >
      ...
      {/* Main Content Area */}
      <div className="flex-1 min-h-0 bg-[#161B22]/50 border border-white/5 rounded-2xl overflow-hidden relative">
  ...
  return (
    <button 
      ...
      className={cn(
        ...
        active ? "bg-[#D95D39] text-white shadow-md shadow-[#D95D39]/20" : "text-white/40 hover:text-white hover:bg-white/5"
      )}
    >
  ...
  return (
    ...
    >
      <table className="w-full text-left border-collapse">
        <thead className="bg-[#161B22] sticky top-0 z-10 border-b border-white/10 text-xs font-bold text-white/40 uppercase tracking-wider">
          <tr>
        ...
        <tbody className="divide-y divide-white/5">
          {orders.map((o) => (
            <tr key={o.id} className="hover:bg-white/5 transition-colors group">
              <td className="px-6 py-4 font-black text-[#D95D39]">{o.id}</td>
              ...
              <td className="px-6 py-4">
                <div className="flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  ...
                  <button onClick={() => onPrint(o)} className="p-2 bg-[#D95D39]/20 hover:bg-[#D95D39]/40 text-[#D95D39] rounded-lg transition-colors" title="Imprimer">
  ...
  return (
    ...
    >
      {KANBAN_COLUMNS.map((colStatus) => {
        ...
        return (
          <div key={colStatus} className="flex-shrink-0 w-72 bg-[#161B22] rounded-2xl flex flex-col border border-white/5 overflow-hidden">
            ...
            <div className="flex-1 p-3 space-y-3 overflow-y-auto scrollbar-thin">
              {colOrders.map((o: any) => (
                <div key={o.id} onClick={() => onViewDetail(o)} className="bg-[#0D1117] border border-white/10 rounded-xl p-3 cursor-pointer hover:border-white/30 transition-all hover:shadow-lg shadow-black/50 group">
                  <div className="flex justify-between items-start mb-2">
                    <span className="text-xs font-black text-[#D95D39]">{o.id}</span>
        ...
        {days.map(d => {
          ...
          return (
            <div key={d} className={cn(
              ...
              isToday ? "border-[#D95D39] bg-[#D95D39]/5" : "border-white/5 bg-[#161B22]"
            )}>
              <span className={cn("text-xs font-bold mb-auto", isToday ? "text-[#D95D39]" : "text-white/60")}>{d}</span>
              {dayOrders > 0 && (
...
function OrderDetailModal({ order, onClose }: { order: PlacedOrder, onClose: () => void }) {
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/60 backdrop-blur-sm">
      <motion.div 
        initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }}
        className="w-full max-w-2xl bg-[#0D1117] border border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
      >
        ...
        <div className="p-6 overflow-y-auto space-y-8 flex-1">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <InfoBlock icon={User} label="Assigné à" value="Salem M. (Caissier)" />
          </div>
            ...
            <div className="space-y-3">
              {order.items?.map((item: any, i: number) => (
                <div key={i} className="flex gap-4 p-3 bg-white/5 rounded-xl border border-white/5">
                  <div className="w-10 h-10 bg-[#161B22] rounded-lg flex items-center justify-center font-black text-[#D95D39] shrink-0 border border-white/10">
                    {item.quantity}x
                  </div>
          ...
          {/* Timeline Simulation */}
          <div>
            <h3 className="text-sm font-bold text-white/40 uppercase tracking-wider mb-3">Timeline</h3>
            <div className="border-l-2 border-white/10 ml-3 space-y-4 py-2">
              <TimelineStep title="Commande créée" time={new Date(order.createdAt).toLocaleTimeString()} active={true} />
              <TimelineStep title="Acceptée en cuisine" time="En cours" active={["cooking", "ready", "delivered"].includes(order.status)} />
              <TimelineStep title="Prête à servir" time="En attente" active={["ready", "delivered"].includes(order.status)} />
              <TimelineStep title="Clôturée" time="En attente" active={order.status === "delivered"} />
            </div>
          </div>
        </div>

        <div className="px-6 py-4 bg-[#161B22] border-t border-white/10 flex justify-between items-center">
          <div className="text-sm text-white/50 font-bold uppercase tracking-wider">Total à payer</div>
          <div className="text-3xl font-black text-[#D95D39]">{formatPrice(order.total)}</div>
        </div>
      </motion.div>
    </div>
  );
...
function InfoBlock({ icon: Icon, label, value }: { icon: any, label: string, value: string }) {
  return (
    <div className="bg-[#161B22] p-3 rounded-xl border border-white/5">
      <Icon className="w-4 h-4 text-[#D95D39] mb-2" />
...
function TimelineStep({ title, time, active }: { title: string, time: string, active: boolean }) {
  return (
    <div className="relative pl-6">
      <div className={cn("absolute left-[-5px] top-1 w-2 h-2 rounded-full ring-4 ring-[#0D1117]", active ? "bg-[#D95D39]" : "bg-white/20")} />
